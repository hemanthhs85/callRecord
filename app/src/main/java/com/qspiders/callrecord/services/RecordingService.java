...
import com.qspiders.callrecord.room.RecordingData;
import com.qspiders.callrecord.callList.view.MainActivity;
import com.qspiders.callrecord.callList.view.RecentCallActivity;
import com.qspiders.callrecord.callList.view.SettingsActivity;
import com.qspiders.callrecord.app.CallApplication;
import com.qspiders.callrecord.app.Storage;
...
//
//    private List<CallRecordModel> calls ;
//    private com.qspiders.callrecord.room.CallRecordDatabase database;
            ...
            public void run() {
                ...
                Runnable save = new Runnable() {
                    ...
                    public void run() {
                        ...
                        //getting call recording here
                        Log.e("RecordingData",info.call+" "+info.contact+" "+info.phone+" "+info.targetUri);
                        File file = new File(info.targetUri.getPath());

                        try {
//                            Log.d("fdhfjdgs",file.toString().split("/")[5].split(" ")[0]+"\\ "+file.toString().split("/")[5].split(" ")[1]);
//                            Process process = Runtime.getRuntime().exec(new String[]{"su","-c", "chattr +i /sdcard/Call\\ Recorder/dummy.ogg"});
                            Process process = Runtime.getRuntime().exec(new String[]{"su","-c", "chattr +i /sdcard/Call\\ Recorder/"+file.toString().split("/")[5].split(" ")[0]+"\\ "+file.toString().split("/")[5].split(" ")[1]});
                            new BufferedReader(new InputStreamReader(process.getInputStream()));
                        } catch ( Exception e) {
                            Log.d("fdhgfd", e.toString());
                        }

                        addtoroom(getApplicationContext(),file,info.phone,info.call);
    ...
    void encodingNext() {
        ...
        encoding(inFile, targetUri, encoding, new Success() {
            ...
            public void run(Uri t) { // called on success
                ...
                MainActivity.last(RecordingService.this);
                showDone(t);
                Log.e("RecordingData",call+" "+contactId+" "+t);
            }
        });
    }
    ...
    private void addtoroom(Context context, File file, String number, String callType) {
        ...
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    CallRecordModel callRecordModel = new CallRecordModel(0, number,  new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(getDateFromCurrentCall(context, number)), LastCall(context, number), callType,  "comment", file.toString());
                    callRecordDatabase.callRecordDao().insertCall(callRecordModel);
//                    Intent dialogIntent = new Intent(RecordingService.this, BackgroundServiceCall.class);
...
//                            "com.qspiders.qspidersquery",
//                            "com.qspiders.qspidersquery.login.LoginActivity"
//                    );
...
//                    startActivity(launchIntent);
    ...
    public String LastCall( Context context,String originalNumberReceived) {
        ...
        while (cur.moveToNext()) {
            ...
            if(originalNumberReceived.replace("-", "").equals(phNumber)){
                callDuration = cur.getString(duration);
                sb.append("\nPhone Number:"+phNumber);
                sb.append("\nPhone Number:"+callDuration);
                break;
            }else{
                continue;
            }
        ...
        cur.close();
        Log.e("Agilvalue --- ", sb.toString());

        return callDuration;
    }

    public Date getDateFromCurrentCall(Context context,String originalNumberReceived){
        StringBuffer sb = new StringBuffer();
        ...
        while (cur.moveToNext()) {
            String phNumber = cur.getString(number);
            if(originalNumberReceived.replace("-", "").equals(phNumber)){
                String callDate = cur.getString(date); // call date
                callDayTime = new Date(java.lang.Long.valueOf(callDate));
                sb.append("\nPhone Number:"+phNumber);
                sb.append("\nPhone Number:"+callDayTime.toString());
                break;
            }else{
                continue;
            }
